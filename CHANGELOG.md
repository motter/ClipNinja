...
When grabbing a new region from inside the annotator, only the
annotator hid — the "Screenshot captured" chooser behind it and the
main window were still on screen and got frozen into the capture,
blocking whatever was behind them. Now the whole window chain hides
(with the same timing the normal capture uses so they're truly gone
before the snapshot), then everything is restored afterward. You can
finally capture what was behind ClipNinja's own windows.
...
Cause: the preview toggled on a raw mouse-up event, and expanding
resized the image *under the cursor* — so the tail of that same click
landed on the newly-grown image and fired a second up-event, undoing
the expand. The preview is now a proper button, whose click fires
exactly once per press-and-release regardless of the resize. A normal
single click expands and stays; click again to shrink.
...
A **Size** dropdown in the annotator toolbar scales the saved/sent
image by a fixed factor: 0.75×, 1.00×, 1.25×, 1.50×, 1.75×, or 2.00×.
A live "→ W×H" hint shows the resulting pixel dimensions. 1.00× is the
default (original size). The editing view stays at full size so your
drawing coordinates never shift; the scale is applied only when you
Save or Send. Because annotations stay vector until then, the image and
your arrows/text are all resampled cleanly at the chosen size rather
than blowing up a finished bitmap.
