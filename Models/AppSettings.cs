...
public class AppSettings
{
    ...
    // Persisted automatically whenever annotations are saved, so the
    // annotator always opens with your last-used setup — pick clay red
    // + large once and it stays that way until you change it.

    /// <summary>Last-used swatch color as #RRGGBB. Must match one of
    /// the annotator's palette entries; unknown values fall back to
    /// Sun Gold.</summary>
    public string AnnotatorDefaultColor { get; set; } = "#F4B844";

    /// <summary>Last-used size: "S", "M", or "L".</summary>
    public string AnnotatorDefaultSize { get; set; } = "M";

    /// <summary>Text-label background style: "dark" (tinted near-black,
    /// the original) or "light" (pale complementary — e.g. red text on
    /// a pale blue field).</summary>
    public string AnnotatorTextStyle { get; set; } = "dark";
