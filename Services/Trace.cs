    ...
    private static void WriterLoop()
    {
        while (true)
        {
            try
            {
                ...
                // Batch up to ~100 lines per file write. _logPath is
                // always initialized before the writer thread starts,
                // but the analyzer can't see that across threads —
                // the null-check with skip keeps it provably safe.
                if (_logPath is not { } logPath) { Thread.Sleep(50); continue; }
